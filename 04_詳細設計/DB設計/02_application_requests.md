...
| 2 | user_id | 依頼顧客ID | BIGINT UNSIGNED |  | ○ |  | `users.id`参照。依頼を行った顧客。 | ON DELETE CASCADE |
...
| セカンダリ | application_requests_user_id_index | user_id | × | 顧客別検索 | Laravel自動命名 |
...
| リレーション | Customer→ApplicationRequest | 1:N、依頼元顧客単位に履歴保持 |  |
